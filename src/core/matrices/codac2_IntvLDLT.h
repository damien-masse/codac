...
namespace codac2
{
  ...
  class IntvLDLT
  {
      ...
      */
     explicit IntvLDLT(const Matrix &M);
      ...
      */
     explicit IntvLDLT(const IntervalMatrix &M);
...
inline const IntervalMatrix &IntvLDLT::matrixLDLT() const {
     assert_release(this->info());
     return this->matrixLDLT_;
}
inline IntervalMatrix IntvLDLT::matrixL() const {
     assert_release(this->info());
     return IntervalMatrix
	(this->matrixLDLT_.triangularView<Eigen::UnitLower>());
}
inline IntervalMatrix IntvLDLT::matrixU() const {
     assert_release(this->info());
     return this->matrixL().transpose();
}
inline IntervalVector IntvLDLT::vectorD() const {
     assert_release(this->info());
     return IntervalVector
	(this->matrixLDLT_.diagonal());
}
inline const Eigen::LDLT<Matrix>::TranspositionType
			&IntvLDLT::transpositionsP() const {
    assert_release(this->info());
    return this->_LDLT.transpositionsP();
}
inline const Eigen::LDLT<Matrix> &IntvLDLT::eigenLDLT() const {
    return this->_LDLT;
}
