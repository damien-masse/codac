...
IntervalMatrix IntLDLT::buildLDLTbounds(const IntervalMatrix &E) {
 ...
 */
IntLDLT::IntLDLT(const Matrix &M) :
   status(0), _LDLT(M), 
 ...
 */
IntLDLT::IntLDLT(const IntervalMatrix &M) :
...
void IntLDLT::computeMatrixLDLT(const IntervalMatrix &M) {
...
bool IntLDLT::info() const {
    return (status==1 || status==-2);
}

BoolInterval IntLDLT::isNegative() const {
...
BoolInterval IntLDLT::isPositive() const {
    if (status==0 || status==-1) return BoolInterval::EMPTY;
    BoolInterval ret = BoolInterval::TRUE;
    for (int i=0;i<matrixLDLT_.cols();i++) {
...
Interval IntLDLT::determinant() const {
...
IntervalMatrix IntLDLT::solve(const IntervalMatrix &rhs) const {
    assert(status!=0);
...
IntervalMatrix IntLDLT::ReconstructedMatrix() const {
