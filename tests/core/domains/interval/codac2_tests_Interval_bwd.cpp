...
void CHECK_AtanOp::bwd(const Interval& y, const Interval& x, const Interval& expected_x)
{
  Interval _x;
...
void CHECK_AddOp::bwd(const Interval& y,
  const Interval& x1, const Interval& x2, const Interval& expected_x1, const Interval& expected_x2)
{
  Interval _x1, _x2;
...
void CHECK_SubOp::bwd(const Interval& y,
  const Interval& x1, const Interval& x2, const Interval& expected_x1, const Interval& expected_x2)
{
  Interval _x1, _x2;
...
TEST_CASE("Interval bwd operations")
{
  ...
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval(.5,2.*sqrt(3.))); CHECK(Approx(x) == Interval(.5,2.));
  a = Interval(PI/6.,PI/3.); y = Interval(.5,2.); x = Interval(.5,10.);            
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval(.5,2.)); CHECK(Approx(x) == Interval(.5,2.*sqrt(3.)));
  a = Interval(-PI/4.,PI/4.); y = Interval(1.,2.); x = Interval(.5,2.);             
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval(1.,2.)); CHECK(Approx(x) == Interval(1.,2.));
  a = Interval(-PI/2.,0.); y = Interval(.5,2.); x = Interval(.5,10.);            
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval::empty()); CHECK(Approx(x) == Interval::empty());
  a = Interval(2.*PI,3.*PI); y = Interval(-.5,2.); x = Interval(.5,10.);            
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval::empty()); CHECK(Approx(x) == Interval::empty());
  a = Interval(2*PI/3.,5.*PI/6.); y = Interval(0.,100.); x = Interval(-20.,-sqrt(3.)/2.); 
  AtanOp::bwd2(a,y,x); CHECK(Approx(y,1e-10) == Interval(.5,20.*sqrt(3.))); CHECK(Approx(x) == Interval(-20.,-sqrt(3.)/2));
  a = Interval(-3*PI/4.,-2*PI/3.); y = Interval(-sqrt(3.)/2.,2.); x = Interval(-sqrt(2.)/2.,0.);   
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval(-sqrt(3.)/2.,0.)); CHECK(Approx(x) == Interval(-sqrt(2.)/2.,0.));
  a = Interval(-3*PI/4.,-2*PI/3.); y = Interval(-sqrt(3.)/2.,2.); x = Interval(-1.,-.5);           
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval(-sqrt(3.)/2.,-.5)); CHECK(Approx(x) == Interval(-sqrt(3.)/2.,-.5));
  a = Interval(-3*PI/4.,-PI/4.); y = Interval(-5.,-.5); x = Interval(-oo,oo);            
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval(-5.,-.5)); CHECK(Approx(x) == Interval(-5.,5.));
  a = Interval(-PI/3.,PI/4.); y = Interval(-oo,oo); x = Interval(sqrt(3.)/2.);       
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == Interval(-1.5,sqrt(3.)/2.)); CHECK(Approx(x) == Interval(sqrt(3.)/2.));
  a = Interval::half_pi(); y = Interval(1); x = 0.;                          
  AtanOp::bwd2(a,y,x); CHECK(Approx(y) == 1.); CHECK(Approx(x) == 0.);
  ...
  CHECK_AddOp::bwd(Interval(1,3),Interval(1,2),Interval(-10,5),Interval(1,2),Interval(-1,2));
  CHECK_AddOp::bwd(Interval::empty(),Interval(0.1,2.0),Interval(-10,2),Interval::empty(),Interval::empty());
  CHECK_AddOp::bwd(Interval(0,0),Interval(0,0),Interval(.1,5),Interval::empty(),Interval::empty());
  CHECK_AddOp::bwd(Interval(0,0),Interval(-1,1),Interval(-1,1),Interval(-1,1),Interval(-1,1));
  CHECK_AddOp::bwd(Interval(-1,1),Interval(1,2),Interval(-10,5),Interval(1,2),Interval(-3,0));

  CHECK_SubOp::bwd(Interval(1,3),Interval(1,2),Interval(-10,5),Interval(1,2),Interval(-2,1));
  CHECK_SubOp::bwd(Interval::empty(),Interval(0.1,2.0),Interval(-10,2),Interval::empty(),Interval::empty());
  CHECK_SubOp::bwd(Interval(0,0),Interval(0,0),Interval(.1,5),Interval::empty(),Interval::empty());
  CHECK_SubOp::bwd(Interval(0,0),Interval(-1,1),Interval(-1,1),Interval(-1,1),Interval(-1,1));
  CHECK_SubOp::bwd(Interval(-1,1),Interval(1,2),Interval(-10,5),Interval(1,2),Interval(0,3));
